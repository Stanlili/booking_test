  ...
  var methods = {
    ...
    _resizeWindow: function _resizeWindow() {
      return this.each(function () {
        ...
        $this.find('.sc_headerPeople_scroll').width(setting.widthTimeX * cellNum);//Stanli 增加人數統計row 22/08/08
    ...
    init: function init(options) {
      return this.each(function () {
        ...
        $this.find('.sc_main_box').on('scroll', function () {
          ...
          $this.find('.sc_headerPeople_scroll').css('left', $(this).scrollLeft() * -1);//Stanli 增加人數統計row 22/08/08
        }); // add time cell
        ...
        for (var t = tableStartTime; t < tableEndTime; t += config.widthTime) {
           {
            htmlPeople = '';
            htmlPeople += '<div class="sc_time">' + "99大99小" + '</div>';
            var $timePeople = $(htmlPeople);
            var cn = Number(Math.min(Math.ceil((t + config.widthTime) / 3600) * 3600, tableEndTime) - t);
            var cellNum = Math.floor(cn / config.widthTime);
